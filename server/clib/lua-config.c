...
lsingleton(lua_State *L) {
	...
	if (G_ARRAY == NULL) {
		...
		if (lua_pcall(L, 2, 1, 0) != LUA_OK) {
			UNLOCK(G_LOCK);
			luaL_error(L,"lsingleton:%s\n",lua_tostring(L,-1));
			return 0;
		}

		G_ARRAY = lua_touserdata(L,-1);

		UNLOCK(G_LOCK);
		return 1;
	} else {
		lua_pushlightuserdata(L,G_ARRAY);
		UNLOCK(G_LOCK);
		return 1;
	}
...
_add(lua_State *L) {
	...
	if (ar->offset == ar->size) {
		ar->size *= 2;
		ar->cfgs = realloc(ar->cfgs,sizeof(struct config) * ar->size);
	}

	ar->elts[ar->offset].value = conf_data;
	ar->elts[ar->offset].key = conf_name;
	memcpy(ar->elts[index-1].key,key,size);
	ar->elts[index-1].key[size] = '\0';
	ar->elts[index-1].size = size;
...
int
luaopen_configarray(lua_State *L) {
	luaL_Reg l[] = {
		{ "singleton",lsingleton },
		{ "new",	lnew },
		{ "delete", ldelete },
		{ "load", lload },
		{ "info",	linfo },
